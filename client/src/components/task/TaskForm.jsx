...
const TaskForm = ({ closeModal, input }) => {
    ...
    const onFinish = async (input) => {
        try {
            ...
            if (input.attachments?.length) {
                ...
                input?.attachments?.forEach((file) => formData.append("images", file));
    ...
    const fetchAllLogData = async (id) => {
        try {
            const { data } = await fetchAllLogs(id);
    ...
    const fetchTaskData = async () => {
        try {
            const { data } = await fetchTaskById(input.key);
