...
const CustomFileUpload: FC<CustomFileUploadProps> = ({ control, name }) => {
        ...
        <Controller
            ...
            render={({ field: { onChange } }) => (
                <div className="file-upload-container">
                    <input type="file" multiple accept="image/*" onChange={(event) => handleFileChange(event, onChange)} className="form-control" />
                    ...
                    <div className="preview-container mt-2 d-flex flex-wrap">
                        {selectedFiles.map((file, index) => (
                            <div key={index} className="preview-item position-relative m-2">
                                ...
                                <button type="button" className="btn btn-danger btn-sm position-absolute top-0 end-0" onClick={() => removeFile(index, onChange)}>
                                    ✕
                                </button>
                            </div>
                        ))}
